...
def collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration):
    ...
    array_record_d = array_record[:, 8 : 18].astype(np.float32)
    if np.max(array_record_d) > 1200 or np.min(array_record_d) < 500:
        print("Wrong value was collected")
...
def pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration):
    ...
    sensor_vals, prev_teensy_1_time, prev_teensy_2_time, weight_array = collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration)
...
def place_back(arm, grip_height, rotate_speed, grasp_height):
    arm.set_position(410, 16.7, 260, 44.9, -87.8, -134.9, speed=rotate_speed, is_radian=False, wait=True)
    ...
    for container in containers:
        ...
        for content in contents:
            ...
            for iteration in range(num_its):     
                ...
                data_1, data_2 = pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration)
                data = [data_1, data_2]
