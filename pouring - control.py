...
def collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration, grip_height, grasp_height, final_weight):
    ...
    while time.time() - start <= p_duration:
        ...
        if time.time() - start >= 0.2:
            if (len(array_record) - 500) % 10 == 0 and count > count_new and not performed_action:
                ...
                w, offset_1, offset_2 = model(cap, torch.tensor(np.array([0]).astype(np.int64)).to(device))
    ...
    print(np.mean(array_record[:500, 8].astype(np.float32)))
    print(np.mean(array_record[:500, 9].astype(np.float32)))
    print(np.mean(array_record[:500, 10].astype(np.float32)))
    print(np.mean(array_record[:500, 11].astype(np.float32)))
    print(np.mean(array_record[:500, 12].astype(np.float32)))
    print(np.mean(array_record[:500, 13].astype(np.float32)))
    print(np.mean(array_record[:500, 14].astype(np.float32)))
    print(np.mean(array_record[:500, 15].astype(np.float32)))
    print(np.mean(array_record[:500, 16].astype(np.float32)))
    print(np.mean(array_record[:500, 17].astype(np.float32)))
    ...
    return array_record, prev_teensy_1_time, prev_teensy_2_time, weight_inter, weight_pre_inter
...
def pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration, final_weight):
    ...
    sensor_vals, prev_teensy_1_time, prev_teensy_2_time, weight_array, weight_pre = collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration, grip_height, grasp_height, final_weight)
    ...
    return sensor_vals, weight_array, weight_pre
...
def trial():
    ...
    with open(r'C:\Users\yexin\Desktop\liquid\ckpts\linear_regression_model_water_final.pkl', 'rb') as file:
        loaded_model = pickle.load(file)
    
    with open(r'C:\Users\yexin\Desktop\liquid\ckpts\poly_features_water_final.pkl', 'rb') as file:
    ...
    for container in containers:
        ...
        for content in contents:
            ...
            for iteration in range(num_its):     
                ...
                data_1, data_2, data_3 = pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration, final_weight)
                data = [data_1, data_2, data_3, final_weight]
