...
def collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration, grip_height, grasp_height, p1_duration, final_weight, weight_num, class_num):
    ...
    while time.time() - start <= p_duration:
        ...
        if time.time() - start >= 1:
            if (len(array_record) - 500) % 5 == 0 and count > count_new and not performed_action:
                
                array_record_p = array_record[500:]
                s = array_record_p[-10:, 8 : 18].astype(np.float32)
                s = (s - 500) / (1000 - 500)
                cap = torch.tensor(s.reshape((1, 10, 10))).to(device)
                class_t = torch.tensor(np.array([class_num]).astype(np.int64)).to(device)
                weight_t = torch.tensor(np.array([weight_num]).astype(np.int64)).to(device)
                # weight_t = torch.tensor(np.array([weight_num])).float().to(device)
                a = model(cap, class_t, weight_t)
            ...
            if (action == 1 or time.time() - start >= p_duration) and not performed_action:
                arm.set_state(4)
                arm.set_state(0)
    ...
    return array_record, prev_teensy_1_time, prev_teensy_2_time, weight_inter
...
def pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration, p1_duration, final_weight, weight_num, class_num):
    ...
    sensor_vals, prev_teensy_1_time, prev_teensy_2_time, weight_array = collect_data_pour(container, content, labels, iteration, rotate_speed, rotate_angle, p_duration, grip_height, grasp_height, p1_duration, final_weight, weight_num, class_num)
    ...
    return sensor_vals, weight_array
    ...
    for container in containers:
        ...
        for content in contents:
            ...
            for iteration in range(num_its):     
                ...
                data_1, data_2 = pick_up_pour(arm, closed_pos, grip_height, iteration, container, content, labels, hold_time, rotate_speed, rotate_angle, grasp_height, p_duration, p1_duration, final_weight, weight_num, class_num)
                data = [data_1, data_2]
